...
func (s *SchemaDomain) WriteTypesHeader(wr io.Writer) {
    tmpl, err := template.New("test").Parse(`#import <PonyDebugger/PDObject.h>
...
{{end}}@property (nonatomic, retain) {{.PropertyType}}{{.PropertyName}};
{{end}}
@end
...
func (s *SchemaDomain) WriteTypesImplementation(wr io.Writer) {
    tmpl, err := template.New("test").Parse(`#import "{{.TypesFileName}}.h"
{{range .Classes}}
...
@property (nonatomic, readonly, retain) {{.ClassName}} *{{.PropertyName}};
...
const Prefix = "PD"
const PathPrefix = "../PonyDebugger/DerivedSources/"
