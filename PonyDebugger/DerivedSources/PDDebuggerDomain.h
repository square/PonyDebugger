...
// Callback Param result: True if debugger supports native breakpoints.
- (void)domain:(PDDebuggerDomain *)domain supportsNativeBreakpointsWithCallback:(void (^)(NSNumber *result, id error))callback;
...
- (void)domain:(PDDebuggerDomain *)domain evaluateOnCallFrameWithCallFrameId:(NSString *)callFrameId expression:(NSString *)expression objectGroup:(NSString *)objectGroup includeCommandLineAPI:(NSNumber *)includeCommandLineAPI returnByValue:(NSNumber *)returnByValue callback:(void (^)(PDRuntimeRemoteObject *result, NSNumber *wasThrown, id error))callback;

@end

@interface PDDebugger (PDDebuggerDomain)

@property (nonatomic, readonly, retain) PDDebuggerDomain *debuggerDomain;
